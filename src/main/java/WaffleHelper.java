...
public class WaffleHelper {
    ...
    public static boolean edge(int x) {
        return x == 0 | x == 7;
    }
    ...
    static int FindLegalMoves(WaffleState state) {
        int x, y;
    ...
    public static void play(String argv[]) throws Exception {
        ...
        if (player1 == 1) {
            ...
            WafflePlayer.FindBestMove(1,board,bestmove);
            ...
            WafflePlayer.FindBestMove(2,board,bestmove);
            if (bestmove[0] != 0) { /* There is a legal move */
                mlen = MoveLength(bestmove);
                PerformMove(board, bestmove, mlen);
                buf = MoveToText(bestmove);
