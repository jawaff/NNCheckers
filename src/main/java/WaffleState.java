...
public class WaffleState {
    ...
    public WaffleState(WaffleState state)
    {
        memcpy(this.board,state.board);
    ...
    public void printMoves() {
        ...
        for (int i = 0; i < numLegalMoves; i++) {
            String move = "";
            for (int j = 0; j < this.movelist[i].length; j++) {
                int position = this.movelist[i][j];
                if (position != 0) {
    ...
    public void shuffleMoves() {
        for (int i = 0; i < numLegalMoves; i++) {
            int newIndex = i + random.nextInt(numLegalMoves - i);
            char[] tmp = this.movelist[i];
            this.movelist[i] = this.movelist[newIndex];
            this.movelist[newIndex] = tmp;
        }

        //printMoves();
    }

    public float[] toNNInputs() {
        float[] inputs = new float[WaffleNNModel.BOARD_SIZE];
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                inputs[i*8 + j] = board[i][j];
            }
        }
        return inputs;
    }
